  ...
  return axios.get(`http://musicbrainz.org/ws/2/recording/${recordingId}?inc=url-rels+work-rels&fmt=json`)
    ...
    .then(work => {
      ...
      if (index + 1 === max) {
        ...
        // console.log(mapped)
        return fs.writeFile('./data.js', JSON.stringify(mapped), 'utf8', (err) => {
...
return axios.get(`http://musicbrainz.org/ws/2/recording?collection=2949d1d4-b6b8-459d-9ae6-1ba131e0b876&fmt=json`)
  .then(response => {
    ...
    return fs.writeFile('./listened.js', JSON.stringify(listened), 'utf8', (err) => {
      if (err) return console.error(err)

      return console.log('Listened recordings saved!')
    })
  })
  // .then(() => axios.get(`http://musicbrainz.org/ws/2/artist/${artistId}?inc=recording-rels&fmt=json`))
  // .then(response => {
